...
int submit_request(int sock, const char *host, uint16_t port, char *resource)
{
    ...
    if (sent_bytes < 0)
    {
        ...
        if ((sock = reopen_socket(host, port)) < 0)
        {
            return sock;
        }
        sent_bytes = send(sock, req, send_sz, 0);
    ...
    //--------------------------------------------------------------------------------
    int content_len = get_http_content_len(recv_buff, sizeof(recv_buff));

    //--------------------------------------------------------------------------------
    // :  Make sure you understand the calculations below
    //
    ...
    // from the server
    //
    // YOUR ANSWER:  we need to loop receiving until there is no more content, so the bytes_remaing
    // will help track how many more bytes we need to receive. Header is part of the received data but
    // not part of the content, so we don't need to count that hence the subtraction.
    //
    //--------------------------------------------------------------------------------
    ...
    //
    // YOUR ANSWER: because we are going to keep the socket alive, we return the socket after
    // processing a request to be re-used for another request.
    //
    //--------------------------------------------------------------------------------
    return sock;
}
